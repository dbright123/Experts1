         ...
         if(OrderType() == OP_BUY){
            if(OrderOpenPrice() > OrderStopLoss()){
               for(int i = 0; i < Bars; i++){
                  zigzag = iCustom(OrderSymbol(),tf,"zigzag",0,i); 
                  if(zigzag != 0 ){
...
void removeTrade(){
   ...
   for(int i  = 0; i < OrdersTotal(); i ++){
      if(OrderSelect(i,SELECT_BY_POS)){
         market = OrderSymbol();
         ema = iMA(market,tf,200,0,MODE_EMA,PRICE_CLOSE,0);
         if(OrderClosePrice() > ema && OrderType() == OP_SELL){
            OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),8);
         }
         else if(OrderClosePrice() < ema && OrderType() == OP_BUY){
            OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),8);
         }
      }
   }
